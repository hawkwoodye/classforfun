...
void * receiver(void * parm)
{
    
    struct recv_parm * input_parms = (struct recv_parm *)parm;
    long tail = 0;
    
    //MPI_Request temp_request;
    MPI_Status recv_status;
    
    struct element * recv_buffer = input_parms->recv_buffer;
    struct program_information prog_info = input_parms->prog_info;
    
    // temp_recv_buffer = input_parms->recv_buffer;
    // temp_final_index = input_parms->_final_index;
    
    int flag = 0;
    int partial_bucket_count = 0;
    int recvd_count;
    long recv_chunk_size = prog_info.input_file_size; //should be communicate_buffer_byte_size
    int my_rank = input_parms-> _my_process;
    // int tags[prog_info.number_of_process];
    
    printf("Rank %d receiver starting...\n", my_rank);
    ...
    while(!flag)
    {
        /*
         ...
         if(flag)
         {
         if(recv_status.MPI_TAG == 255)
         {
         tags[recv_status.MPI_SOURCE] = 255;
         if(tag_checking(tags, prog_info.number_of_process))
         {
         MPI_Recv(tags, 0, MPI_BYTE, recv_status.MPI_SOURCE, recv_status.MPI_TAG, MPI_COMM_WORLD, &recv_status);
         break;
         }else{
         MPI_Recv(tags, 0, MPI_BYTE, recv_status.MPI_SOURCE, recv_status.MPI_TAG, MPI_COMM_WORLD, &recv_status);
         printf("%d get tag 255 from %d\n", my_rank, recv_status.MPI_SOURCE);
         flag=0;
         continue;
         }
         }
        ...
        MPI_Recv(&recv_buffer[tail], recv_chunk_size, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &recv_status);
        MPI_Get_count(&recv_status, MPI_BYTE, &recvd_count);
        printf("Rank %d received %ld records from %d\n", my_rank, recvd_count/100, recv_status.MPI_SOURCE);
        tail += recvd_count/100;
        if (recv_status.MPI_TAG == DIETAG){
            partial_bucket_count++;
            if (partial_bucket_count == prog_info.number_of_process)
                break;
        }
    }
    printf("Rank %d receiver done!\n", my_rank);
    return NULL;
}
...
int main(int argc, char* argv[]) {
    ...
    // *final_index = 0;
    struct recv_parm recv_thread_parm;
    
    // recv_thread_parm._status = probe_status;
    recv_thread_parm.recv_buffer = final_distributed_records;
    recv_thread_parm.prog_info = prog_info;
    // recv_thread_parm._final_index = final_index;
    recv_thread_parm._my_process= my_rank;
    
    pthread_t recv_thread;
    pthread_create(&recv_thread, NULL, receiver, (void*)&recv_thread_parm);
    ...
    for(i = 0; i < num_process; i++)
    {
        printf("%d sending DIETAG to %d!\n", my_rank, i);
        MPI_Send(0, 0, MPI_BYTE, i, DIETAG, MPI_COMM_WORLD);
    }
